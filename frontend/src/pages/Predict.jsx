...
const labelStyle = { fontSize: 12, color: '#cbd5e1', display: 'block', marginBottom: 4 };

export default function Predict() {
  const initForm = { Education: 'Graduate', Living_With: 'Alone' };
  ...
  const handleSubmit = async () => {
    ...
    if (!form.Age || Number(form.Age) < 18 || Number(form.Age) > 89) {
      setError('Please enter a valid Age (18–89).'); return;
    }
    ...
    const payload = Object.fromEntries(
      Object.entries(form).map(([k, v]) =>
        ['Education', 'Living_With'].includes(k)
          ? [k, v]
          : [k, Number(v) || 0]
      )
    );
        ...
        <Card>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: '1rem' }}>
            <div>
              ...
              <select style={inputStyle} value={form.Education} onChange={e => handleChange('Education', e.target.value)}>
            ...
            <div>
              <label style={labelStyle}>Living With</label>
              <select style={inputStyle} value={form.Living_With} onChange={e => handleChange('Living_With', e.target.value)}>
            ...
            {FIELDS.map(f => (
              <div key={f.key}>
                ...
                <input
                  ...
                  style={{
                    ...inputStyle,
                    border: error && (f.key === 'Income' || f.key === 'Age') && !form[f.key]
                      ? '1px solid #f87171'
                      : '1px solid #374151'
                  }}
                  value={form[f.key]}
          ...
          {/* Required note */}
          <p style={{ fontSize: 11, color: '#6b7280', marginBottom: '0.75rem' }}>
            <span style={{ color: '#f87171' }}>*</span> Income and Age are required fields
          </p>
        ...
        {result && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{
              ...
              <p style={{ fontSize: 12, color: '#cbd5e1', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>
                Predicted Segment
              </p>
              <div style={{
                ...
                justifyContent: 'center', fontSize: 24, color: result.color, fontWeight: 700
              }}>
                {result.cluster}
              </div>
